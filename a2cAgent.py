...
import gym
import gym_ple
from rl_framework import FNAgent, Trainer, Observer
...
class BrakeOutObserver(Observer):
...
class ActorCriticTrainer(Trainer):
    ...
    def train(self, env, episode_count=12000, initial_count=10,
...
def main(play, is_test, notebook=False):
...
#         env = gym.make("Breakout-v0")
        #BOX(210,160,3)
#         obs = BrakeOutObserver(env, 60, 80, 4)
        trainer.learning_rate = 7e-5

    if play:
        agent = agent_class.load(obs, path)
        agent.play(obs, episode_count=10, render=True, notebook=notebook)
    else:
